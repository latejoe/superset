...
test('renders with default props', async () => {
  ...
  expect(document.querySelector('.anticon-calendar')).toBeInTheDocument();
});
...
test('renders since and until with specific date/time with default locale', () => {
  ...
  expect(document.querySelectorAll('.anticon-calendar').length).toBe(2);
});
...
test('should translate Date Picker', async () => {
  ...
  await waitForElementToBeRemoved(() => screen.queryByLabelText('Loading'));
  userEvent.click(
    document.querySelectorAll('.anticon-calendar')[0] as HTMLElement,
  );
...
test('calls onChange when START Specific Date/Time is selected', async () => {
  ...
  userEvent.click(calendarIcons[0] as HTMLElement);
...
test('calls onChange when END Specific Date/Time is selected', async () => {
  ...
  const calendarIcons = document.querySelectorAll('.anticon-calendar');
  userEvent.click(calendarIcons[1] as HTMLElement);
